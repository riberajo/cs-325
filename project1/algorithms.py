...
def recursive_msa(left_bound=0, right_bound=0, array=[]):
    ...
    right_max = recursive_msa(left_bound+int(floor((right_bound-left_bound)/2))+1, right_bound, array)
    print "Calling recursive_msa from ", left_bound, " to ", left_bound+int(floor((right_bound-left_bound)/2)), " for left max"
    left_max = recursive_msa(left_bound, left_bound+int(floor((right_bound-left_bound)/2)), array)
...
def recursive_msa_call(array=[]):
    start_idx = 0
    end_idx = 0
    max = recursive_msa(0,len(array)-1, array)
